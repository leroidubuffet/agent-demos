...
Antes de empezar (no es necesario si no vamos a ejecutar)
...
La forma más rápida de ir a los archivos es usar el buscador.
...
Abre agent-demos/01-minimal-agent/src/main/java/com/example/agent/v1
/CodeReviewAgent.java. Di: "esto es el contrato del agente. Solo una interfaz Java. LangChain4j genera la implementación que gestiona el loop completo."

Abre 01-minimal-agent/src/main/java/com/example/agent/v1
/Main.java. Señala el bloque del builder:
...
Aquí le damos un diff hardcodeado mínimo. No tiene acceso al repositorio ni puede leer archivos todavía.
...
Abre agent-demos/01-minimal-agent/src/main/java/com/example/agent/v2
/GitTools.java. Señala la anotación @Tool y lee la descripción en voz alta: "Lee el contenido completo de un archivo. Usar cuando necesites ver código existente antes de comentarlo o referenciarlo." Di: "esta descripción es lo que el modelo lee para decidir si invocar la herramienta. Como ya sabemos, una descripción vaga produce invocaciones incorrectas."

Abre agent-demos/01-minimal-agent/src/main/java/com/example/agent/v2
/Main.java. Señala la única línea añadida respecto a v1:

.tools(new GitTools())

Esto es todo lo que cambia en el código del cliente. Esto permite al modelo llamar a herramientas como readFile para leer archivos reales del repo, de manera que puede buscar contexto adicional.

En la práctica el flujo es:

Se pasa el diff hardcodeado al agente
El modelo ve que el diff menciona TaskNotFoundException
Decide invocar readFile("...TaskNotFoundException.java") para ver si esa clase existe y entender el contexto
Con ese contexto añadido, genera el feedback
...
Abre agent-demos/01-minimal-agent/src/main/java/com/example/agent/v3
/GitTools.java
. Señala el nuevo método listFiles. Di: "ahora el agente puede explorar el directorio antes de leer un archivo, sin que nadie le diga la ruta exacta."
...
Abre agent-demos/02-spring-ai-comparison/src/main/java/com/example/springai
/CodeReviewTools.java. Señala @Component. Di: "en Spring AI las tools son beans de Spring. Pueden usar @Autowired, acceder a JPA, a Security, a cualquier parte del ecosistema Spring."

Abre agent-demos/02-spring-ai-comparison/src/main/java/com/example/springai
/CodeReviewRunner.java y muestra el comentario al inicio del archivo. Lee las dos columnas en voz alta:
...
Desde /agent-demos (raiz del proyecto), ejecuta los dos y compara las trazas:
...
Tools por agente
15
3 (revisor) / 2 (documentador)
Tools duplicadas
readFile + getFileContent (hacen lo mismo)
Sin solapamiento
System prompt
Ninguno
Uno por agente, con rol explícito
Tiempo
22 s
69 s
Calidad del output
Superficial: "leí el archivo y los tests pasan"
Análisis detallado + documentación en Jira/Markdown
Por qué
El modelo completa la tarea mecánicamente y para
El rol fuerza al modelo a opinar, no solo a ejecutar


La lección clave: el tiempo extra del FocusedAgent no es un coste - es el modelo trabajando de verdad. El TooManyToolsAgent fue más rápido porque hizo menos. Un espacio de decisión grande y sin rol definido produce respuestas mínimas; roles acotados producen respuestas de calidad.
...
La descripción enumera cuándo usar la tool y cuándo no
El ejemplo de ruta en el @P ayuda al modelo a formatear el argumento correctamente
El bloque try/catch devuelve un mensaje útil: "No se puede leer '...'. Prueba listar el directorio primero." El modelo puede reaccionar a eso
